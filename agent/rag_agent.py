...
PROMPT_TEMPLATE_TXT_AGENT = config['prompts']['template_path_agent']
PROMPT_TEMPLATE_TXT_GRADE = config['prompts']['template_path_grade']
PROMPT_TEMPLATE_TXT_GENERATE = config['prompts']['template_path_generate']
PROMPT_TEMPLATE_TXT_REWRITE = config['prompts']['template_path_rewrite']
...
class ToolConfig:
    ...
    def __init__(self, tools):
        ...
        self.tool_names = {tool.name.lower() for tool in tools}
    ...
    def _build_routing_config(self, tools):
        ...
        for tool in tools:
            ...
            # 红旗/分诊：不评分，直接生成（避免误判导致 rewrite 循环）
            if tool_name in {"retrieve_redflag", "retrieve_triage"}:
                routing_config[tool_name] = "generate"
                logger.debug(f"Tool '{tool_name}' routed to 'generate' (redflag/triage)")
                continue
            # 知识检索：评分
            if "retrieve" in tool_name:
                routing_config[tool_name] = "grade_documents"
                logger.debug(f"Tool '{tool_name}' routed to 'grade_documents' (knowledge retrieval)")
            else:
                routing_config[tool_name] = "generate"
                logger.debug(f"Tool '{tool_name}' routed to 'generate' (non-retrieval)")
...
class ParallelToolNode(ToolNode):
    ...
    def __call__(self, state: dict) -> dict:
        ...
        tool_map = {tool.name: tool for tool in self.tools} # type: ignore
...
def store_memory(question: BaseMessage, config: RunnableConfig, store: BaseStore) -> str:
    ...
    try:
        ...
        # 如果包含“记住”，存储新记忆
        if "记录" in question.content.lower(): # type: ignore
            memory = escape(question.content) # type: ignore
...
def agent(state: MessagesState, config: RunnableConfig, *, store: BaseStore, llm_chat, tool_config: ToolConfig) -> dict:
    ...
    try:
        ...
        # 自定义线程内存储逻辑 过滤消息
        messages = filter_messages(state["messages"]) # type: ignore

        # 将工具绑定到 LLM
        llm_chat_with_tool = llm_chat.bind_tools(tool_config.get_tools())

        # 创建代理处理链
        agent_chain = create_chain(llm_chat_with_tool, PROMPT_TEMPLATE_TXT_AGENT)
        # 调用代理链处理消息
...
def grade_documents(state: MessagesState, llm_chat) -> dict:
    ...
    try:
        ...
        # 创建评分处理链
        grade_chain = create_chain(llm_chat, PROMPT_TEMPLATE_TXT_GRADE, DocumentRelevanceScore)
        # 调用评分链评估相关性
...
def rewrite(state: MessagesState, llm_chat) -> dict:
    ...
    try:
        ...
        question = get_latest_question(state)
        # 创建重写处理链
        rewrite_chain = create_chain(llm_chat, PROMPT_TEMPLATE_TXT_REWRITE)
        # 调用重写链生成新查询
...
def generate(state: MessagesState, llm_chat) -> dict:
    ...
    try:
        ...
        # 创建生成处理链
        generate_chain = create_chain(llm_chat, PROMPT_TEMPLATE_TXT_GENERATE)
        # 调用生成链生成回复
...
def route_after_tools(state: MessagesState, tool_config: ToolConfig) -> Literal["generate", "grade_documents"]:
    ...
    try:
        ...
        # 检查消息是否来自已注册的工具
        tool_name = (last_message.name or "").lower()
...
def create_graph(db_connection_pool: ConnectionPool, llm_chat, llm_embedding, tool_config: ToolConfig) -> StateGraph:
    ...
    try:
        # 创建Postgres检查点保存实例
        checkpointer = PostgresSaver(db_connection_pool) # type: ignore
        # 初始化检查点
        checkpointer.setup()
    except Exception as e:
    ...
    try:
        ...
        store = PostgresStore(db_connection_pool, index={"dims": 768, "embed": llm_embedding}) # type: ignore
        store.setup()
    except Exception as e:
    ...
    # 添加代理节点
    workflow.add_node("agent", lambda state, config: agent(state, config, store=store, llm_chat=llm_chat, tool_config=tool_config)) # type: ignore
    # 添加工具节点，使用并行工具节点
    workflow.add_node("call_tools", ParallelToolNode(tool_config.get_tools(), max_workers=5))
    # 添加重写节点
    workflow.add_node("rewrite", lambda state: rewrite(state,llm_chat=llm_chat)) # type: ignore
    # 添加生成节点
    workflow.add_node("generate", lambda state: generate(state, llm_chat=llm_chat)) # type: ignore
    # 添加文档相关性评分节点
    workflow.add_node("grade_documents", lambda state: grade_documents(state, llm_chat=llm_chat)) # type: ignore
...
def graph_response(graph: StateGraph, user_input: str, config: dict, tool_config: ToolConfig) -> None:
    ...
    try:
        # 启动状态图流处理用户输入
        events = graph.stream({"messages": [{"role": "user", "content": user_input}], "rewrite_count": 0}, config) # type: ignore
        # 遍历事件流
        for event in events:
            # 遍历事件中的值
...
def main():
    ...
    try:
        ...
        db_connection_pool = ConnectionPool(conninfo=DB_URI, max_size=20, min_size=2, kwargs=connection_kwargs, timeout=10)
        ...
        monitor_thread = monitor_connection_pool(db_connection_pool, interval=60) # type: ignore

        # 创建状态图
        try:
            graph = create_graph(db_connection_pool, llm, llm_embedding, tool_config) # type: ignore
