...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        llm_chat, llm_embedding = get_llm(config["llm"]["type"])
        ...
        db_connection_pool = ConnectionPool(
            conninfo=config['database']['db_uri'],
            max_size=20,
            min_size=2,
        ...
        try:
            # 使用数据库连接池和模型创建状态图
            graph = create_graph(db_connection_pool, llm_chat, llm_embedding, tool_config) # type: ignore
...
async def get_dependencies() -> Tuple[any, any]: # type: ignore
    """
    依赖注入函数，用于获取 graph 和 tool_config。
...
async def chat_completions(request: ChatCompletionRequest, dependencies: Tuple[any, any] = Depends(get_dependencies)): # type: ignore
    """接收来自前端的请求数据进行业务的处理。
...
if __name__ == "__main__":
    logger.info(f"Start the server on port {config['server']['port']}...")
